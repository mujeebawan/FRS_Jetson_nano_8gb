...
async def get_current_settings(
    ...
    """Get current runtime settings including attendance settings."""
    detector = request.app.state.detector
    recognizer = request.app.state.recognizer
    stream = request.app.state.stream
    ...
    return {
        ...
        "checkout_gap_hours": settings.checkout_gap_hours,
        "standard_work_hours": settings.standard_work_hours,
        "attendance_cooldown_seconds": settings.attendance_cooldown_seconds,
        "save_attendance_snapshot": settings.save_attendance_snapshot,

        "ranges": {
            "detection_confidence": {"min": 0.1, "max": 1.0, "step": 0.05, "default": 0.5},
            "recognition_threshold": {"min": 0.1, "max": 1.0, "step": 0.05, "default": 0.4},
            "frame_skip": {"min": 1, "max": 10, "step": 1, "default": 1},
            "late_threshold_minutes": {"min": 5, "max": 60, "step": 5, "default": 15},
            "checkout_gap_hours": {"min": 1, "max": 8, "step": 0.5, "default": 4},
            "standard_work_hours": {"min": 4, "max": 12, "step": 0.5, "default": 8},
            "attendance_cooldown_seconds": {"min": 30, "max": 300, "step": 30, "default": 60}
        }
    }
