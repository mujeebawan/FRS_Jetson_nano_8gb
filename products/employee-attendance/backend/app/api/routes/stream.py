"""Stream API routes with attendance WebSocket support"""
...
_attendance_websockets: list[WebSocket] = []


def broadcast_attendance_sync(data: dict):
    """
    Synchronously broadcast attendance event to all connected WebSocket clients.
    Called from the attendance callback in main.py.
    """
    import asyncio
    ...
    async def _broadcast():
        ...
        disconnected = []
        for ws in _attendance_websockets:
            try:
                await ws.send_text(message)
            except Exception:
                disconnected.append(ws)
        for ws in disconnected:
            if ws in _attendance_websockets:
                _attendance_websockets.remove(ws)
...
async def websocket_attendance(websocket: WebSocket):
    ...
    Clients receive check-in/check-out events as they happen.
    """
    await websocket.accept()
    _attendance_websockets.append(websocket)
    logger.info(f"Attendance WebSocket connected. Total: {len(_attendance_websockets)}")
    ...
    try:
        ...
        logger.debug(f"Attendance WebSocket error: {e}")
    finally:
        if websocket in _attendance_websockets:
            _attendance_websockets.remove(websocket)
        logger.info(f"Attendance WebSocket disconnected. Total: {len(_attendance_websockets)}")
