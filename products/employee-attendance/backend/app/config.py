...
class Settings(BaseSettings):
    ...
    # Attendance Settings (with persistent defaults)
    late_threshold_minutes: int = _persisted.get("late_threshold_minutes", 15)  # Minutes after shift start = late
    checkout_gap_hours: float = _persisted.get("checkout_gap_hours", 4.0)  # Hours gap to trigger checkout
    standard_work_hours: float = _persisted.get("standard_work_hours", 8.0)  # Standard work day
    attendance_cooldown_seconds: int = _persisted.get("attendance_cooldown_seconds", 60)  # Prevent duplicate logs
    save_attendance_snapshot: bool = _persisted.get("save_attendance_snapshot", True)  # Save check-in/out photos

    @property
    def attendance_snapshots_dir(self) -> str:
        """Attendance snapshots directory."""
        return str(self._project_root / "data" / "attendance_snapshots")
