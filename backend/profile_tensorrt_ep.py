...
# Set library path BEFORE importing onnxruntime
...
def profile_with_tensorrt_ep(
    ...
    # Warmup (first runs build TRT engines)
    logger.info("Warmup (TRT engine building on first run)...")
    for i in range(20):
        _ = app.get(test_image)
        if i == 0:
            logger.info("First inference complete")
    ...
    results = {
        ...
        'p50_latency_ms': float(np.percentile(inference_times, 50)),
...
def print_results(results: dict):
    ...
    print("\n" + "="*60)
    print("TENSORRT EP PROFILING RESULTS")
    print("="*60)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="TensorRT EP Face Recognition Profiler")
    ...
    results = profile_with_tensorrt_ep(
